...
def get_event_list(request):
    ...
    if eid != '':
        event = {}
        try:
            result = Event.objects.get(od=eid)
...
def get_guest_list(request):
    ...
    if eid == '':
        return JsonResponse({'status': 10021, 'message': 'did cannot be empty'})

    if eid != '' and phone == '':
        datas = []
        resultes = Guest.objects.filter(event_id=eid)
        if resultes:
            for r in resultes:
                guest = {}
...
def user_sign(request):
    ...
    result = Event.objects.get(id=eid).statis
    if not result:
        return JsonResponse({'status': 10023,
                             'message': 'event status is not available'})
